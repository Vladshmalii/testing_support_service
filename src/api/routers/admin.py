...
async def get_statistics(current_admin: Dict[str, Any] = Depends(require_admin)):
    ...
    return await AdminService.get_statistics(admin_id)
...
async def get_all_users(
    ...
    admin_id = current_admin["user_id"]
    return await AdminService.get_all_users(admin_id, pagination)
...
async def get_all_staff(
    ...
    admin_id = current_admin["user_id"]
    return await AdminService.get_all_staff(admin_id, pagination)
...
async def delete_user(
    ...
    admin_id = current_admin["user_id"]
    return await AdminService.delete_user(admin_id, user_id)
...
async def delete_staff(
    ...
    admin_id = current_admin["user_id"]
    return await AdminService.delete_staff(admin_id, staff_id)
...
async def get_staff_workload(current_admin: Dict[str, Any] = Depends(require_admin)):
    admin_id = current_admin["user_id"]
    return await AdminService.get_staff_workload(admin_id)
...
@router.get(
    "/requests/export",
    dependencies=[
        Depends(PermissionsValidator([Permissions.EXPORT_DATA]))
    ]
)
async def export_requests_csv(
    ...
    admin_id = current_admin["user_id"]
    return await CSVService.export_requests_csv(filters, admin_id)